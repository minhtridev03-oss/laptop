...
export default function Checkout() {
      ...
      <section className="luxury-page-section mx-auto min-h-[70vh] w-full max-w-[900px] px-4 py-14 lg:px-6">
        ...
        <div className="luxury-panel rounded-[10px] px-6 py-14 text-center sm:px-12">
          ...
          <div className="mx-auto my-8 grid max-w-md gap-3 rounded-md border border-border-subtle bg-bg-main/70 p-5 text-sm">
            ...
            <div className="flex justify-between gap-4"><span className="text-text-muted">Tổng thanh toán</span><strong className="font-['Be_Vietnam_Pro'] text-text-main">{formatCommercePrice(order.total)}</strong></div>
          </div>
          {order.payment_config && form.paymentMethod === 'bank_transfer' && (
            <div className="mx-auto mb-8 max-w-md rounded-lg border border-primary/20 bg-primary/[0.05] p-5 text-left">
              <h2 className="font-['Be_Vietnam_Pro'] text-sm font-bold text-text-main">Thông tin chuyển khoản</h2>
  ...
  return (
    <>
      <Helmet><title>{t('checkout.title')} | Laptop World</title></Helmet>
      <section className="luxury-page-section mx-auto min-h-[70vh] w-full max-w-[1200px] px-4 py-10 lg:px-6 lg:py-14">
        <Link to="/cart" className="mb-6 inline-flex min-h-10 items-center gap-2 text-xs font-semibold uppercase tracking-[0.08em] text-text-muted hover:text-primary-hover"><ChevronLeft size={16} aria-hidden="true" /> {t('common.back')}</Link>
        <div className="mb-8 border-b border-border-subtle pb-6">
          <p className="luxury-eyebrow mb-3">{t('checkout.eyebrow')}</p>
          <h1 className="luxury-heading text-3xl">{t('checkout.title')}</h1>
        </div>
          ...
          <form onSubmit={handleSubmit}
            ...
            <div className="luxury-panel rounded-[10px] p-5 sm:p-7">
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <Field label={t('checkout.phone')} name="phone" value={form.phone} onChange={updateField} autoComplete="tel" inputMode="tel" pattern="[0-9+ .-]{8,15}" required />
                <Field label={t('checkout.email')} name="email" value={form.email} onChange={updateField} autoComplete="email" type="email" />
                <Field label={t('checkout.city')} name="province" value={form.province} onChange={updateField} autoComplete="address-level1" required />
                <div className="sm:col-span-2"><Field label={t('checkout.address')} name="address" value={form.address} onChange={updateField} autoComplete="street-address" required /></div>
                <label className="sm:col-span-2"><span className="mb-2 block text-xs font-semibold text-text-main">{t('checkout.notes')}</span><textarea name="notes" value={form.notes} onChange={updateField} rows="4" className="luxury-search w-full rounded-md px-4 py-3 text-sm text-text-main outline-none" placeholder="" /></label>
              </div>
            ...
            <aside className="luxury-panel rounded-[10px] p-6 lg:sticky lg:top-[108px]"
              ...
              <button type="submit" disabled={submitting} className="luxury-primary-button mt-6 flex min-h-12 w-full items-center justify-center gap-2 rounded-md px-5 text-xs font-bold uppercase tracking-[0.08em] disabled:cursor-wait disabled:opacity-60">
                <ShieldCheck size={17} aria-hidden="true" /> {submitting ? t('checkout.processing') : t('checkout.place_order')}
              </button>
            </aside>
          </form>
        )}
